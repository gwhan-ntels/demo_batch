...
public class NBlpyf02m02JobLauncher {
	
	/** logger. */
	private static final Logger log = LoggerFactory.getLogger(NBlpyf02m02JobLauncher.class);
	...
	public static void main(String[] args) {
		ApplicationContext context = new ClassPathXmlApplicationContext("spring/batch-nBlpyf02m02-tasklet.xml");
		JobLauncher jobLauncher = (JobLauncher) context.getBean("jobLauncher");
		Job job = (Job)context.getBean("batch-nBlpyf02m02-tasklet");
		
		String[] billFields = {"billSeqNo" , "prodCmpsId" , "svcCmpsId" , "billYymm" , "billCycl" , "billDt" , "soId" , "billMmTp" , "grpId" , "pymAcntId" , "custId" , "ctrtId" , "prodCd" , "svcCd" , "svcGrp" , "atrtCorpId" , "atrtEmpId" , "taxTp" , "custTp" , "crncyCd" , "exrate" , "exrateAplyDt" , "payDueDt" , "billMdmGiroYn" , "billMdmEmlYn" , "billMdmSmsYn" , "billMdmFaxYn" , "payMthd" , "smlBsAmt" , "zipCd" , "billAddr" , "smry" , "billAmt" , "useStrtDt" , "useEndDt"};
		
		Field[] billField = CBillComm.class.getDeclaredFields();
		
		for (String name : billFields) {
			
			boolean find = false;
			
			for (Field field : billField) {
		...
		clogService.setProcessName("[NBlpyf02m02]");
