...
public class NBlivb01m04Writer02 implements ItemWriter<CBillComm>, StepExecutionListener {
	...
	public int SetDivByAmt(int procType, double dDivVal)
	{
		...
		//System.out.println("SetDivByAmt : " + procType+ "  -----------------------------------------");
...
//				System.out.println("Objectlist : BILL_SEQ_NO -" + addItem.getBillSeqNo() 
...
//	            + " PRE_ADJ - " + addItem.getAdjPrvBillAmt().toString());
//
//				System.out.println("ObjectDivlist : BILL_SEQ_NO -" + newItem.getBillSeqNo() 
...
//                + " PRE_ADJ - " + newItem.getAdjPrvBillAmt().toString());
	...
	public int SetDivProc() throws Exception    
	{
		...
		if ( ObjCtrtList.size() <= 0 ) return 1;
		...
		switch ( strTmpSpltPymTp )
		{
			case "1" :
				...
				if ( strItllmt.equals("1") == true      &&  dTotSum >= dDivAmt ) nret = SetDivByAmt(1, dDivAmt );
				else if ( strItllmt.equals("2") == true &&  dTotSum <  dDivAmt ) nret = SetDivByAmt(2, dDivAmt );
				else nret = SetDivByAmt(3, dDivAmt );				
		    break;
		...
		if ( nret >= 0 ) nret = clsService.saveDivJdbcDirect(ObjectDivlist);
...
//	    System.out.println("Objectlist ----------------------------------------------------------");
	...
	public ExitStatus afterStep(StepExecution stepExecution) {
		...
		try {
			...
			if ( nret < 0 ) clslog.writeLog("SetDivProc Error!");
			else clslog.writeLog("SerDivProc Success !");
		} catch (Exception e) {
