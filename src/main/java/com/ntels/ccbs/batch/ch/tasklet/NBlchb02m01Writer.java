	...
	public void write(List<? extends NBlchb02m01> items) throws Exception {
		...
		for (NBlchb02m01 list : items) {
			...
			for (int i = 0; i < listRRate.size(); i++) {
				...
				if (list.getRateItmCd().equals(listRRate.get(i).getRateItmCd())) {
					bCompCondChk = false;
					bCompFctrChk = false;
					...
					if (!listRRate.get(i).getFctrNo().equals(listRRate.get(i).getFctrCnt())) {
						...
						System.err.println("rateMap = " + rateMap.toString());
						System.err.println("rateMap = " + rateMap.size());

						for (int j = 0; j < rateMap.size(); j++) {
							Long rateNo = (long) j + 1;
							rateInfo = (RateInfo) rateMap.get(rateNo);

							System.err.println("rateNo = " + rateNo);
							...
							System.out.println("[" + listRRate.size() + "]=>[" + i + "] matchCnt = [" + matchCnt + "]-[" + listFctrCd + "]----" + rateFctrCd + "=>" + listFctrVal);
...
//								rateVal = Double.parseDouble(getFctrVal(list, fctrCd));
								
								System.err.println("fctrCd = " + fctrCd);
								System.err.println("prod_cd = " + list.getProdCd());
					        ...
					        * 	
					        */
							list.setCrncyCd(listRRate.get(i).getCrncyCd());
					
							if (listRRate.get(i).getQtyRefFl().equals("0")) {
								rateVal = rateVal * listRRate.get(i).getQty();
								list.setUseQty(Double.valueOf(listRRate.get(i).getQty()));
							} else if (listRRate.get(i).getQtyRefFl().equals("1")) {
								String qtyFctrCd = listRRate.get(i).getQtyFctrCd();

								System.err.println("qtyFctrCd  = " + qtyFctrCd);
								if (!getFctrVal(list, qtyFctrCd).equals("*")) {
									rateVal = rateVal * Double.parseDouble(getFctrVal(list, qtyFctrCd));
									//rateVal = rateVal * Integer.parseInt(getFctrVal(list, qtyFctrCd));
									list.setUseQty(Double.parseDouble(getFctrVal(list,qtyFctrCd)));
								}
							}
...
//							double procRate = Double.valueOf((String) list.getProcRate());
			...
			//20200205 임시로 일회성 RI003의 경우에는 사용년월과 과금개시월이 같은 경우에만 처리하는 프로세스 추가 
			//최종 계산값을 리스트에 저장한다. 맵핑 카운트 초기화
			if (bFinalChk && bCompFctrChk
					&& (("RI003".equals(list.getRateItmCd()) && (list.getUseYymm().equals(list.getRateStrtDt().substring(0, 6))) || !("RI003".equals(list.getRateItmCd())) )) ) {
