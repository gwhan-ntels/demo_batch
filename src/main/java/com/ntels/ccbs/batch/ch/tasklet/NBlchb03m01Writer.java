...
public class NBlchb03m01Writer implements ItemWriter<NBlchb03m01>, StepExecutionListener {
	...
	public void write(List<? extends NBlchb03m01> items) throws Exception {
		...
		for (NBlchb03m01 list : items) {
			...
			System.out.println("["+ list.getCtrtId() + ":"+list.getProdCd()+"]=====" + prevSvcRateItmTypCd + "]:[" + prevRateItmCd+ "]=====>" + prevTotAmt + ": " + prevTotDisAmt);
			
			if (prevSvcCmpsId.equals("First")) { //최초 대상이 없는 경우 계약, 상품id를 채워준다.
				prevSvcCmpsId = list.getSvcCmpsId();
				prevRateItmCd = list.getRateItmCd();
				// SHLEE 20170921 수정 
				// prevSvcRateItmTypCd = list.getSvcRateItmTypCd();  => prevSvcRateItmTypCd = list.getRsvcRateItmTypCd();
				prevSvcRateItmTypCd = list.getRsvcRateItmTypCd();
				///   		
			...
			} else if (!prevRateItmCd.equals(list.getRateItmCd()) || !prevSvcRateItmTypCd.equals(list.getRsvcRateItmTypCd())) {
				// prevSvcRateItmTypCd = list.getSvcRateItmTypCd();  => prevSvcRateItmTypCd = list.getRsvcRateItmTypCd();
				...
				System.out.println("Before =========================["+ prevSvcRateItmTypCd + ":"+list.getRsvcRateItmTypCd()+"]=====" + prevTotDisAmt);
				// SHLEE 20170921 수정 
			  // getSvcRateItmTypCd() => getRsvcRateItmTypCd()
				if (!prevSvcRateItmTypCd.equals(list.getRsvcRateItmTypCd())) { //요금항목이 바뀌면 할인내역으로 등록
					prevSvcRateItmTypCd = list.getRsvcRateItmTypCd();
					bSvcRateChk = true;
					prevTotDisAmt = 0.0;
			  ...
			  // 
				System.out.println("After==========================["+ prevSvcRateItmTypCd + ":"+list.getRsvcRateItmTypCd()+"]=====" + prevTotDisAmt);
					...
					if (!listDRate.get(i).getFctrNo().equals(listDRate.get(i).getFctrCnt())) {
						...
						if (bFinalChk && bCompFctrChk) {
							...
							if (listDRate.get(i).getRateRefFl().equals("0")) {
								...
								System.err.println("getFctrVal(list, fctrCd) = " + getFctrVal(list, fctrCd));
								
								rateVal = Double.parseDouble(getFctrVal(list, fctrCd));
								list.setUseQtyUt(String.valueOf(rateVal));
							}
							
							list.setCrncyCd(listDRate.get(i).getCrncyCd());
