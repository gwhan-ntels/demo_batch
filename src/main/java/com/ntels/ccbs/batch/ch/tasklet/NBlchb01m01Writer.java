...
public class NBlchb01m01Writer implements ItemWriter<NBlchb01m01>, StepExecutionListener {
		...
		for (NBlchb01m01 list : items) {
			...
			if (skip == 0) {
				...
				while (CUtil.getIntCompdate(list.getUseEdDate(), prevMulti.getEndDate()) < 0) {
					NBlchb01m01 prevList = new NBlchb01m01();
					CUtil.copyObjectValue(list, prevList);

//					if (list.getInactDt().equals("99991231")) {
