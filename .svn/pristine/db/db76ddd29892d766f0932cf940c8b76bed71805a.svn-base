...
import com.ntels.ccbs.batch.ch.service.NBlchb09m01Service;
import com.ntels.ccbs.batch.common.CUtil;

/**
 * <PRE>
 * 1. ClassName: NBlchb09m01Writer
 * 2. FileName : NBlchb09m01Writer.java
...
@Component
@Scope("step")
public class NBlchb09m01Writer implements ItemWriter<NBlchb09m01>, StepExecutionListener {
	Logger log = LoggerFactory.getLogger(this.getClass());

	String stepName;
	ItemWriter<NBlchb09m01> itemWriter;

	@Autowired
	private NBlchb09m01Service nBlchb09m01Service;
	...
	public void write(List<? extends NBlchb09m01> items) throws Exception {
		double vat = 0.0;
		
		objectlist.clear();

		for (NBlchb09m01 list : items) {
			if (list.getUseAmt() != 0) {
				//부가세 계산
		...
		nBlchb09m01Service.saveCharge(objectlist);
		nBlchb09m01Service.updateThrwy(objectlist);
		nBlchb09m01Service.saveSvcCmpsMt(objectlist);
	}
