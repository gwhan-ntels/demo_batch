...
@Component
public class NBlivb01m32Writer02 implements ItemWriter<CBillComm>, StepExecutionListener {
...
//	public void write(List<? extends NBliv32m01> lists) throws Exception {
	public void write(List<? extends CBillComm> items) throws Exception {
		...
		for(CBillComm list : items){
			...
			list.setRegrId("NBLIV01M32");
			Objectlist.add(list);
		}
	
		
		clsService.saveMastJdbcDirect(Objectlist);
		if ( i == 10000) {
			System.out.println("================================================================");
